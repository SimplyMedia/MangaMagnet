 ...
 */
import { ChapterMetadataResponse, ChapterResponse, MangaResponse } from "@/services/openapi";
...
export function VolumeOverview({manga}: { manga: MangaResponse }) {
	...
	useEffect(() => {
		const pairs = manga.chapterMetadata.map<ChapterData>(metadata => ({
			metadata,
			local: manga.chapters.find(c => c.id === metadata.id)!,
			mangaId: manga.id,
		}));
	...
	return (
		<div>
			{items.map(i => (
				<VolumeList key={i[0].metadata.volumeNumber} chapters={i} volume={i[0].metadata.volumeNumber}/>
			))}
		</div>
	)
}
